...
| Skill | 何する | 主な用途 | 自動発火する言い回しの例 |
|---|---|---|---|
| `voice-write` | 指定人格の声で文章を書く・既存AI文章を書き直す | AI臭を抜く本命。記事・SNS投稿・既存原稿のリライト | 「ENFPで書いて」「INTJ風の記事」「人間っぽく書き直して」「humanize this」 |
| `voice-decide` | 指定人格として選択肢から1つ決める（中立禁止） | 自分の頭の中にいないタイプがどう判断するかを擬似的に覗く。企画初動の30分の壁打ち | 「ENTJで決めて。A) ... B) ...」「INFPならどっち選ぶ」 |
| `voice-vote` | 指定人格群（または16人格全員）で賛成/反対/保留を投票し可決判定 | 提案を出す前のリハーサル。実在チームメンバーの人格を割り当ててバーチャル会議 | 「これ通せそう？」「チームに投票させて」「ステークホルダーシミュ」 |
| `voice-ask` | 指定人格があなたに鋭い質問を返す（答えない、問う） | プレゼンの質疑応答対策・アイデア出しの壁打ち | 「ENTPでツッコんで」「INFJに問い詰めて」「このアイデア叩いて」 |
...
設計の背景・3原則の中身・人格別の出力比較は [`Outputs/qiita-article-voice-skills.md`](Outputs/qiita-article-voice-skills.md)（Qiita寄稿用の解説記事）にまとめてあります。
...
# 育児アプリの次の新機能を ISTJ に決めさせる
ISTJで決めて。A) AIで離乳食の写真から栄養を解析 B) 夫婦でリアルタイム共有できる育児ログ C) 発達マイルストーン通知

# 定例MTG削減案を、実在チームを模した5人格で投票
チームに投票させて: 定例MTGを週2回から週1回に減らす --types=entj,istj,entp,esfj,intp

# プレゼン資料に ENTJ から鋭い質問を返してもらう
ENTJで質疑応答対策して: <ここにスライドの要点を貼る>

# マネタイズ計画を ENTP に問い詰めてもらう
ENTPでツッコんで: 個人開発のプラグインを来年中にマネタイズしたい
```
